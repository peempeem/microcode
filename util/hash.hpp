...
template <class V> Hash<V>::Hash(unsigned minSize, unsigned minLoad, unsigned maxLoad) : _size(0)
{
    _minLoad = (minLoad < 0 || minLoad > 50) ? 25 : minLoad;
    _maxLoad = (maxLoad < _minLoad || maxLoad > 100) ? 75 : maxLoad;
    
    if (minSize != -1)
    {
        _prime = 1;
        while ((primes[_prime] * _maxLoad) / 100 < minSize)
            _prime += 2;
        _minPrime = _prime;
    }
    else
...
template <class V> void Hash<V>::reserve(unsigned size)
{
    ...
    while ((primes[prime] * _maxLoad) / 100 < size)
        prime += 2;
    _minPrime = prime;
    _realloc(prime);
}
...
template <class V> void Hash<V>::shrink()
{
    ...
    while (newPrime > 2 && newPrime > _minPrime && _size < (primes[newPrime] * _minLoad) / 100)
        newPrime -= 2;
    _realloc(newPrime);
}
...
{
    if (_size + 1 > (_table->size() * _maxLoad) / 100)
        _realloc(_prime + 2);
...
template <class V> void Hash<V>::_remove(unsigned idx, bool canResize)
{
    ...
    if (canResize && _size < (_table->size() * _minLoad) / 100)
    {
...
template <class V> typename StaticHash<V>::Itterator StaticHash<V>::begin()
{
    ...
    {
        if ((*_probe)[i].used)
            return Itterator(i, _table, _probe);
    }
    return end();
}
...
template <class V> void StaticHash<V>::_realloc(unsigned newPrime)
{
    ...
    std::vector<Node>* table = new std::vector<Node>(primes[_prime]);
    std::vector<Probe>* probe = new std::vector<Probe>(primes[_prime]);
...
template <class V> unsigned StaticHash<V>::_contains(unsigned key)
{
    ...
    do
    {   
        if ((*_probe)[idx].used && (*_table)[idx].key == key)
            return idx;
        if (!(*_probe)[idx].probe)
            break;
        idx = (idx + h2) % _table->size();
...
template <class V> void StaticHash<V>::_copy(const StaticHash& other)
{
    ...
    _prime = other._prime;
    _table = new std::vector<Node>(primes[_prime]);
    _probe = new std::vector<Probe>(*other._probe);
}