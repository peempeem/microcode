...
std::vector<uint16_t> GridGraph::connections(uint16_t node)
{
    std::vector<uint16_t> conn;
    uint16_t* idx = _toIDX.at(node);
    if (idx)
    {
        Vertex& v = _vertices[*idx];
        conn.reserve(v.edges.size());
        for (Vertex::Edge& e : v.edges)
            conn.push_back(_toID[e.id]);
    }
    return conn;
}
...
void GridGraph::representTable(NetworkTable& table)
{
    ...
    for (auto it = table.nodes.begin(); it != table.nodes.end(); ++it, ++i)
    {
        ...
        for (unsigned j = 0; j < (*it).connections.size(); ++j)
        {
            ...
            {
                v.edges[j].id = *id;
                v.edges[j].weight = (10000 * (conn.usage * 8 + conn.linkspeed)) / conn.linkspeed;
            }
            else
                v.edges[j].id = -1;
        }
